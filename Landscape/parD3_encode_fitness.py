...
def evaluate_model(genotype_space, fitness_space, available_ratio, degree=1, n_repeats=10, vis=False, ml='LR'):
    ...
    # metrics_mean_std = {
    #     'R2_mean': np.mean(results[:, 0]),
    #     'R2_std': np.std(results[:, 0]),
    #     'MAPE_mean': np.mean(results[:, 1]),
    #     'MAPE_std': np.std(results[:, 1]),
    #     'NMSE_mean': np.mean(results[:, 2]),
    #     'NMSE_std': np.std(results[:, 2])
    # }
    
    # if vis:
    #     plt.figure(figsize=(4, 4))
    #     plt.scatter(y_test, y_pred, color='blue', alpha=0.25)
    #     plt.plot([-0.2, 1], [-0.2, 1], color='red', linestyle='--')
    #     plt.text(0.1, 0.9, f'R² = {metrics_mean_std["R2_mean"]:.2f}', fontsize=12)
    #     plt.text(0.1, 0.8, f'MAPE = {metrics_mean_std["MAPE_mean"]:.2f}', fontsize=12)
...
def train(dim, device):
    ...
    conf = OmegaConf.load('config/parD3.yaml')
    conf.device = device
    conf.model.feature_dim = dim
    conf.log_dir = conf.log_dir + f'd{conf.model.feature_dim}/'

    from simulate.parD3 import Dataset_parD3
    batch_size = conf.train.batch_size
    train_dataset = Dataset_parD3(conf)
    ...
    from simulate.parD3 import getSimTraj
    import numpy as np

    trajectorys = getSimTraj(conf, vis=False, save=False, traj_num=100, force=True)
    trajectorys = np.array(trajectorys)  # (N, T, L)
    trajectorys_onehot = np.eye(conf.parD3.states)[trajectorys.astype(int)]  # (N, T, L, nstates)
    trajectorys_onehot = torch.tensor(trajectorys_onehot).float().to(conf.device)

    import pickle
    with open('data/parD3/system.pkl', 'rb') as f:
        system = pickle.load(f)
    trajectorys_fitness = np.array([system.get_fitness(genotype) for genotype in trajectorys.reshape(-1, conf.parD3.loci)]).reshape(trajectorys.shape[:2])
    ...
    metrics = compute_metrics(trajectorys, trajectorys_hat, num_classes=conf.parD3.states) # N×T×(accuracy, precision, recall, f1)
...
def test(dim, device):
    ...
    # Genotype space
    loci, states = conf.parD3.loci, conf.parD3.states
    ...
    for available_ratio in [0.025, 0.05, 0.075, 0.1, 0.125, 0.15, 0.175, 0.2, 0.225, 0.25]:
        for ml in ['LR', 'SVR']:
            if os.path.exists(conf.log_dir + f'result_{ml}_{available_ratio}.npy'):
                continue
            ...
            # print(f'ML: {ml} | Available Ratio: {available_ratio:.2f}')
            # print(f'R²: {result["R2_mean"]:.2f} ± {result["R2_std"]:.2f}')
            # print(f'MAPE: {result["MAPE_mean"]:.2f} ± {result["MAPE_std"]:.2f}')
            # print(f'NMSE: {result["NMSE_mean"]:.2f} ± {result["NMSE_std"]:.2f}')
...
def test_origin():
    ...
    for available_ratio in [0.025, 0.05, 0.075, 0.1, 0.125, 0.15, 0.175, 0.2, 0.225, 0.25]:
        for ml in ['LR', 'SVR']:
            ...
            np.save(conf.log_dir + f'y_test_original_{ml}_{available_ratio}.npy', y_test)
            np.save(conf.log_dir + f'y_pred_original_{ml}_{available_ratio}.npy', y_pred)
    ...
    for ml in ml_list:
        ...
        for dim in dims:
            for available_ratio in available_ratios:
                ...
                if os.path.exists(file_path):
                    ...
                    r2_mean, r2_std = np.mean(results[:, 0]), np.std(results[:, 0])
                    mape_mean, mape_std = np.mean(results[:, 1]), np.std(results[:, 1])
        ...
        for available_ratio in available_ratios:
            ...
            if os.path.exists(reference_file_path):
                reference_results = np.load(reference_file_path, allow_pickle=True)
                reference_r2_mean = np.mean(reference_results[:, 0])
                reference_r2_std = np.std(reference_results[:, 0])
                reference_mape_mean = np.mean(reference_results[:, 1])
                reference_mape_std = np.std(reference_results[:, 1])
