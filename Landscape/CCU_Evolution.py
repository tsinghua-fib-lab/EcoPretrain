...
traj_onehot = np.eye(conf.CCU.states)[traj.astype(int)]  # (N, T, L, nstates)

dim_list = [8, 12, 16, 20, 24, 28, 32, 36, 40]
...
predictor = GeneTypePredictor(conf.CCU.loci, conf.CCU.states, ml='SVM')

L, b, T = conf.CCU.loci, conf.CCU.states, conf.CCU.steps
...
metrics = compute_metrics(y_test[:, 1:], ml_predictions, num_classes=conf.CCU.states) # N×(T-1)×(accuracy, precision, recall, f1)
...
predictor = GeneTypePredictor(conf.CCU.loci, conf.CCU.states, ml='SVM')
random_predictions = predictor.random_predict(initial_state.shape[0], conf.CCU.steps)

random_metrics = compute_metrics(y_test[:, 1:], random_predictions, num_classes=conf.CCU.states)
...
for dim_idx, dim in enumerate(dim_list):
    ...
    z_train_norm = scaler.transform(z_train.reshape(-1, feature_dim)).reshape(-1, conf.CCU.steps, feature_dim)
    z_test_norm = scaler.transform(z_test.reshape(-1, feature_dim)).reshape(-1, conf.CCU.steps, feature_dim)
    ...
    for t in range(1, conf.CCU.steps):
        ml_predictions_z[:, t] = predictor.predict(ml_predictions_z[:, t-1])

    ml_predictions_z = scaler.inverse_transform(ml_predictions_z.reshape(-1, feature_dim)).reshape(-1, conf.CCU.steps, feature_dim)
    z_test_y = scaler.inverse_transform(z_test_norm.reshape(-1, feature_dim)).reshape(-1, conf.CCU.steps, feature_dim)
    ...
    metrics = compute_metrics(y_test[:, 1:], x_pred[:, 1:], num_classes=conf.CCU.states) # N×(T-1)×(accuracy, precision, recall, f1)
    metrics_our[dim_idx].append(metrics)
    
    os.makedirs('figs/fig3/', exist_ok=True)
    np.save(f'figs/fig3/CCU_z_pred_{dim}.npy', ml_predictions_z)
    np.save(f'figs/fig3/CCU_x_pred_{dim}.npy', x_pred)
    np.save(f'figs/fig3/CCU_z_gt_{dim}.npy', z_test_y)
    np.save(f'figs/fig3/CCU_x_gt.npy', y_test)
        

np.save('figs/fig3/CCU_baseline.npy', metrics_baseline)
np.save('figs/fig3/CCU_our.npy', metrics_our)
np.save('figs/fig3/CCU_random.npy', metrics_random)
