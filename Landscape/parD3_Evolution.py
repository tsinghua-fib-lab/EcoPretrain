...
class GeneTypePredictor:
    def __init__(self, L, b, ml='SVM'):
        ...
        if ml == 'SVM':
            ...
            self.classifiers = [RandomForestClassifier(random_state=42) for _ in range(L)]
...
traj_onehot = np.eye(conf.parD3.states)[traj.astype(int)]  # (N, T, L, nstates)

dim_list = [4, 12, 20, 28, 36, 44, 52, 60]
...
predictor = GeneTypePredictor(conf.parD3.loci, conf.parD3.states, ml='SVM')

L, b, T = conf.parD3.loci, conf.parD3.states, conf.parD3.steps
...
metrics = compute_metrics(y_test[:, 1:], ml_predictions, num_classes=conf.parD3.states) # N×(T-1)×(accuracy, precision, recall, f1)
...
predictor = GeneTypePredictor(conf.parD3.loci, conf.parD3.states, ml='SVM')
random_predictions = predictor.random_predict(initial_state.shape[0], conf.parD3.steps)

random_metrics = compute_metrics(y_test[:, 1:], random_predictions, num_classes=conf.parD3.states)
...
for dim_idx, dim in enumerate(dim_list):
    ...
    z_train_norm = scaler.transform(z_train.reshape(-1, feature_dim)).reshape(-1, conf.parD3.steps, feature_dim)
    z_test_norm = scaler.transform(z_test.reshape(-1, feature_dim)).reshape(-1, conf.parD3.steps, feature_dim)
    ...
    for t in range(1, conf.parD3.steps):
        ml_predictions_z[:, t] = predictor.predict(ml_predictions_z[:, t-1])

    ml_predictions_z = scaler.inverse_transform(ml_predictions_z.reshape(-1, feature_dim)).reshape(-1, conf.parD3.steps, feature_dim)
    z_test_y = scaler.inverse_transform(z_test_norm.reshape(-1, feature_dim)).reshape(-1, conf.parD3.steps, feature_dim)
    ...
    metrics = compute_metrics(y_test[:, 1:], x_pred[:, 1:], num_classes=conf.parD3.states) # N×(T-1)×(accuracy, precision, recall, f1)
    metrics_our[dim_idx].append(metrics)
    
    os.makedirs('figs/fig3/', exist_ok=True)
    np.save(f'figs/fig3/parD3_z_pred_{dim}.npy', ml_predictions_z)
    np.save(f'figs/fig3/parD3_x_pred_{dim}.npy', x_pred)
    np.save(f'figs/fig3/parD3_z_gt_{dim}.npy', z_test_y)
    np.save(f'figs/fig3/parD3_x_gt.npy', y_test)
    

np.save('figs/fig3/parD3_baseline.npy', metrics_baseline)
np.save('figs/fig3/parD3_our.npy', metrics_our)
np.save('figs/fig3/parD3_random.npy', metrics_random)
