...
class AdaptiveWalkSimulation:
    ...
    def get_fitness(self, genotype):
        # return self.fitness_table[genotype[0], genotype[1], genotype[2]]
...
def getSimTraj(conf, vis=True, save=True, traj_num=None, force=False):
    ...
    loci, states, steps = conf.parD3.loci, conf.parD3.states, conf.parD3.steps
    traj_num = traj_num or conf.parD3.traj_num # if traj_num is None, use conf.parD3.traj_num
    fitness_table = np.load(conf.data_dir + "preprocessed_fitness_table.npy", allow_pickle=True)[..., 0] 
    ...
    system = AdaptiveWalkSimulation(
        ...
        population_size=conf.parD3.population_size,
        mutation_rate=conf.parD3.mutation_rate,
...
class Dataset_parD3(Dataset):
    ...
    def load_data(self, traj):
        ...
        self.nstates = self.conf.parD3.states
...
if __name__ == '__main__':
    ...
    trajectorys = getSimTraj(conf, vis=False, save=False, traj_num=1000, force=True)